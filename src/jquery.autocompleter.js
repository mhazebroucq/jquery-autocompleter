...
(function (global, factory) {
    ...
    function _build($node, opts) {
        if (!$node.hasClass("autocompleter-node")) {
            ...
            html += '<ul class="autocompleter-list">';
            if (opts.headerTemplate) {
                html += opts.headerTemplate;
            }
            html += '</ul>';
            html += "</div>";
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < data.limit) {
                        ...
                        var value = data.customValue && source[item][data.customValue] ? source[item][data.customValue] : source[item].value;

                        switch (i) {
                            case 0:
                                if (label.toUpperCase().search(query) === 0 || value.toUpperCase().search(query) === 0) {
                            ...
                            case 1:
                                if (label.toUpperCase().search(query) !== -1 || value.toUpperCase().search(query) !== -1 ) {
                                    response.push(source[item]);
                                    delete source[item];
                                }
    ...
    function _clear(data) {
        ...
        if (data.headerTemplate) {
            data.$autocompleter.find(".autocompleter-list").html(data.headerTemplate);
        } else {
    ...
    function _buildList(list, data) {
        ...
        data.response = list;
        var content = data.$autocompleter.find(".autocompleter-list").html();
        data.$autocompleter.find(".autocompleter-list").html(content + menu);
